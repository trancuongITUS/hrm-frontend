...
@Component({
    ...
    template: `
        <div class="relative inline-block">
            <p-avatar [image]="image()" [icon]="icon()" [label]="label()" [size]="primeSize()" [shape]="shape()" [class]="customClass()" [ariaLabel]="ariaLabel()" />

            @if (showStatus()) {
                <span class="absolute bottom-0 right-0 rounded-full ring-2 ring-white dark:ring-surface-900" [class]="statusClass()" [attr.aria-label]="status() + ' status'"></span>
            }
        </div>
    `,
    styles: []
