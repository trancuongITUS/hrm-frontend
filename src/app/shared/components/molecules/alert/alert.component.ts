    ...
    template: `
        @if (visible()) {
            <div [class]="containerClass()">
                <p-message [severity]="severity()" [text]="text()" [icon]="computedIcon()" [class]="'w-full'">
                    @if (!text()) {
                ...
                @if (dismissible()) {
                    <button type="button" class="ml-auto text-current opacity-70 hover:opacity-100 transition-opacity" (click)="handleDismiss()" [attr.aria-label]="'Dismiss alert'">
